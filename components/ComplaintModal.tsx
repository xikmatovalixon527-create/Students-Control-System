  ...
  useEffect(() => {
    ...
    if (!template) {
      if (category === 'homework') {
        if (lang === 'ru') {
          text = 'Здравствуйте, {ParentName}! Напоминаем, что {StudentName} выполнил(а) домашнее задание всего на {Value}%. Просим Вас проконтролировать подготовку к занятиям.';
        } else {
          text = "Assalomu alaykum, {ParentName}! {StudentName} uy vazifasini bor-yo'g'i {Value}% ga bajardi. Iltimos, darsga tayyorgarlikni nazoratga oling.";
        }
      } else if (category === 'lateness') {
        if (lang === 'ru') {
          text = 'Здравствуйте, {ParentName}! Сообщаем, что {StudentName} опоздал(а) на урок на {Value} мин. Пожалуйста, обеспечьте своевременный приход на занятия.';
        } else {
          text = "Assalomu alaykum, {ParentName}! {StudentName} darsga {Value} daqiqa kechikib keldi. Iltimos, darslarga vaqtida kelishini ta'minlang.";
        }
      } else if (category === 'absence_reason') {
        if (lang === 'ru') {
          text = 'Здравствуйте, {ParentName}! Зафиксировано отсутствие {StudentName} на уроках по уважительной причине ({Value}). Желаем скорейшего возвращения!';
        } else {
          text = "Assalomu alaykum, {ParentName}! {StudentName} darsda uzurli sabab ({Value}) tufayli qatnasha olmadi. Tezroq darslarga qaytishini tilaymiz!";
        }
      } else if (category === 'absence_no_reason') {
        if (lang === 'ru') {
          text = 'Здравствуйте, {ParentName}! {StudentName} отсутствует на занятиях без предупреждения. Пожалуйста, свяжитесь с нами для уточнения причин.';
        } else {
          text = "Assalomu alaykum, {ParentName}! {StudentName} darsda sababsiz qatnashmadi. Iltimos, biz bilan bog'laning.";
        }
      } else if (category === 'overdue') {
        if (lang === 'ru') {
          text = 'Здравствуйте, {ParentName}! Напоминаем, что у {StudentName} имеется просрочка по оплате обучения на {Value}. Пожалуйста, внесите оплату в ближайшее время. Если вы уже оплатили, проигнорируйте это сообщение. Спасибо!';
        } else {
          text = "Assalomu alaykum, {ParentName}! {StudentName} ning o'quv to'lovi bo'yicha {Value} kechikish borligini eslatib o'tamiz. Iltimos, to'lovni yaqin orada amalga oshiring. Agar to'lagan bo'lsangiz, bu xabarni inobatga olmang. Rahmat!";
        }
      } else {
        if (lang === 'ru') {
          text = 'Здравствуйте, {ParentName}! Сообщение касательно {StudentName}: {Value}';
        } else {
          text = 'Assalomu alaykum, {ParentName}! {StudentName} haqida xabar: {Value}';
        }
      }
    } else {
